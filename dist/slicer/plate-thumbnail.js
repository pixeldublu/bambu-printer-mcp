 ...
 */
export async function injectPlateThumbnailsIfMissing(threeMfPath, sourceModelPath) {
    let tempPath;
    try {
...
async function loadStlTriangles(stlPath) {
...
function renderIsometric(triangles, size) {
    ...
    const drawable = projected.map((triangle) => ({
...
function fillTriangle(rgba, size, triangle) {
