...
export function selectFilamentsForSlice(trays, currentSlot, options = {}) {
    ...
    if (hasExplicitSlots) {
        ...
        const selected = slots.map((slot) => {
            ...
            if (requestedType && actualType !== requestedType) {
                throw new Error(`Material mismatch: requested ${requestedType}, but AMS slot ${slot} contains ${actualType || "UNKNOWN"}.`);
            }
    ...
    const eligible = trays.filter((tray) => tray.loaded && tray.slot !== null && Boolean(tray.resolved_profile_path));
    if (requestedType) {
        const matching = eligible.filter((tray) => normalizeMaterialType(tray.tray_type) === requestedType);
        if (matching.length === 0) {
            throw new Error(`No loaded AMS filament matches requested material ${requestedType}. Loaded slots: ${describeAvailable(trays)}.`);
        }
    ...
    if (eligible.length === 0)
        return null;
    const materialTypes = new Set(eligible.map((tray) => normalizeMaterialType(tray.tray_type) || "UNKNOWN"));
    if (materialTypes.size !== 1) {
        throw new Error(`Ambiguous loaded AMS materials (${Array.from(materialTypes).join(", ")}). Pass filament_type or ams_slots; loaded slots: ${describeAvailable(trays)}.`);
    }
